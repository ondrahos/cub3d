...
/*   Updated: 2024/06/19 19:15:11 by ohosnedl         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		while (map_tab[i][++j])
		{
			...
			if (!(ft_strchr("10NSEW", map_tab[i][j])))
				return (print_error(game->map.path,
						"Invalid character in map", FAIL));
			if (ft_strchr("NSEW", map_tab[i][j]) && game->player.dir != '0')
				return (print_error(game->map.path,
						"More than one player in map", FAIL));
			if (ft_strchr("NSEW", map_tab[i][j]) && game->player.dir == '0')
...
static int	check_player_position(t_game *game, char **map_tab)
{
	...
	if (game->player.dir == '0')
		return (print_error(game->map.path,
				"Map has no player spawning position", FAIL));
	i = -1;
	while (map_tab[++i])
	{
		j = -1;
...
int	check_map(t_game *game, char **map_tab)
{
	...
	if (check_map_walls_around(&game->map, map_tab) == FAIL)
		return (print_error(game->map.path,
				"Map is not surrounded by walls", FAIL));
	if (game->map.height < 3)
		return (print_error(game->map.path,
				"Map is too small, should be at least 3 lines high", FAIL));
	...
	if (check_map_is_last_element(&game->map) == FAIL)
		return (print_error(game->map.path,
				"Map is not the last element in file", FAIL));
	return (SUCC);
}
